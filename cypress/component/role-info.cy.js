...
describe("role-info", () => {
    ...
    it("visually switches between roles in one run", () => {
        mountWithSelect();

        cy.get("role-info").shadow().find(".box")
            .should("contain.text", "Selecteer een rol om de beschrijving te zien.");

        cy.get("#roleSelect").select("speler");
        cy.get("role-info").shadow().find(".box")
            .should("have.text", "Speler: Neem deel aan quizzen, beantwoord vragen en bekijk je scores.");

        cy.wait(800);

        cy.get("#roleSelect").select("quizmaster");
        cy.get("role-info").shadow().find(".box")
            .should("have.text", "Quizmaster: Maak en beheer quizzen, start lobbies en beheer spelers.");

        cy.wait(800);

        cy.get("#roleSelect").select("-- kies rol --");
        cy.get("role-info").shadow().find(".box")
            .should("contain.text", "Selecteer een rol om de beschrijving te zien.");
    });
});